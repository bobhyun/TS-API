...
- [TS-API 프로그래밍 안내서](#ts-api-프로그래밍-안내서)
  ...
  - [저장 데이터 검색](#저장-데이터-검색)
    ...
    - [차량 번호 로그 내보내기 `@0.9.18`](#차량-번호-로그-내보내기-0918)
...
여기서부터는 요청 URL의 `auth=` 부분을 간결하게 생략하여 표현합니다. 참고로 v0 (레거시) API는 응답 JSON에서 항상 **절대 URL** (`http://{host}/...`)을 반환합니다 — `image`, `videoSrc`, `src` 등의 필드는 scheme과 host를 포함합니다. (v1은 상대 URL을 반환합니다.)
```ruby
# 매개변수
lang            # 자막 언어 지정
...
### 차량 번호 로그 내보내기 `@0.9.18`
검색한 차량 번호 로그를 엑셀 파일 변환하여 다운로드 받을 수 있습니다.
```ruby
...
```jsx
[
  "http://192.168.0.100/download/2018-01-01T00.00.00.xls",   // 엑셀 파일 다운로드 주소
  "http://192.168.0.100/download/2018-01-01T00.00.00(2).xls",
    // ... 중략
]
```
  ...
  "data": [
    {
      ...
      "image": "http://192.168.0.100/storage/e/0/0/7/7673/object/7673911/7673911.4412659.1590386295819221.object._c1_t2_s392x504.jpg" # 이미지 주소
    },
    {
      ...
      "image": "http://192.168.0.100/storage/e/0/0/7/7673/object/7673911/7673911.4412660.1590386294449281.object._c1_t2_s744x624.jpg" # 이미지 주소
    },
    {
      ...
      "image": "http://192.168.0.100/storage/e/0/0/7/7673/object/7673911/7673911.4412658.1590386294185653.object._c1_t1_s296x464.jpg" # 이미지 주소
    },
    {
      ...
      "image": "http://192.168.0.100/storage/e/0/0/7/7673/object/7673909/7673909.4412630.1590386121557955.object._c1_t3_s440x312.jpg" # 이미지 주소
    },
    // ... 중략
  ]
}
...
```jsx
{
  ...
  "chid": {                                     // 차량 번호 인식 채널
    ...
    "title":"카메라1",
    "displayName":"CH1. 카메라1",                // TS-API@0.9.4 이후 추가됨
    "src":"http://192.168.0.100/watch?ch=1&when=2018%2D06%2D27T10%3A42%3A06%2E575%2B09%3A00"  // 차량 번호 인식 시점의 영상
  },
  ...
  "linkedChannel": [                            // 연동된 채널
    {
      ...
      "src":"http://192.168.0.100/watch?ch=2&when=2018%2D06%2D27T10%3A42%3A06%2E575%2B09%3A00" // 차량 번호 인식 시점의 영상
    }
  ],
  "image":"http://192.168.0.100/storage/e/0/0/0/39/39612.161192.1576732638241699.plate.jpg", // 캡쳐된 낱장 이미지
  ...
  "linkedChannel":[                             // 연동된 채널
    {
      ...
      "title":"카메라1",
      "displayName":"CH1. 카메라1",               // TS-API@0.9.4 이후 추가됨
      "src":"http://192.168.0.100/watch?ch=1"
    },
    {
      "chid":2,
      "title":"카메라2",
      "displayName":"CH2. 카메라2",               // TS-API@0.9.4 이후 추가됨
      "src":"http://192.168.0.100/watch?ch=2"
    }
  ],
...
```jsx
{
  ...
  "image": "http://192.168.0.100/storage/e/0/0/7/7673/object/7673854/7673854.4411971.1590382562558672.object._c1_t2_s552x400.jpg" # 이미지 주소
}
```
2. `face` 객체 속성
    | key         | 형식       | 목록                                  |
...
```jsx
{
  ...
  "image": "http://192.168.0.100/storage/e/0/0/7/7673/object/7673854/7673854.4411974.1590382578461739.object._c1_t1_s376x600.jpg" # 이미지 주소
}
```
2. `human` 객체 속성
    | key         | 형식              | 목록                            |
...
1. 예제 데이터
{
  ...
  "image": "http://192.168.0.100/storage/e/0/0/7/7673/object/7673880/7673880.4412174.1590384238768802.object._c1_t3_s944x520.jpg" # 이미지 주소
}
```
2. `vehicle` 객체 속성
    | key           | 형식          | 목록                            |
...
# 백업 데이터 암호화
password        # v0.9.20.에 포함됨 (비밀번호는 base64 인코딩)
    ...
    "file": {
      ...
      "download": [
        # 생성된 동영상 파일
        {
          ...
          "src": "http://192.168.0.100/download/7963635e-1bff-40e1-bbf3-3f17525aef40/CH1.2018-07-27T09.11.19.mp4",
          "md5": "1125ee2c3d20f30b31166c821204603d" # md5=true로 요청시 포함됨
        },
        # 생성된 자막 파일
        {
          "fileName": "CH1.2018-07-27T09.11.19.vtt",
          "src": "http://192.168.0.100/download/7963635e-1bff-40e1-bbf3-3f17525aef40/CH1.2018-07-27T09.11.19.vtt",
...
**프리셋 추가**
카메라의 현재 위치를 프리셋으로 추가합니다.

프리셋을 추가하면 카메라에서 프리셋 토큰이 생성됩니다.
옵션으로 `name`을 지정할 수 있습니다. 카메라가 다양한 문자를 지원하지 않으므로 가능하면 영문자와 숫자로 지정하길 권장합니다.
`name`을 지정하지 않으면 카메라에서 생성된 프리셋 토큰과 동일한 이름으로 저장됩니다.
...
# 프리셋 신규 추가시 (name 생략한 경우)
GET /api/channel/preset?ch=1&add
```


**프리셋 지정**
기존 설정된 프리셋을 현 위치로 변경 저장합니다.

프리셋을 설정하기 위해서는 두 개의 매개변수가 필요합니다.
첫 번째 매개변수는 `프리셋 토큰`, 두 번째 매개변수는 `name`입니다.
`name`은 이름을 변경할 경우만 지정하면 됩니다.
카메라가 다양한 문자를 지원하지 않으므로 `name`은 가능하면 영문자와 숫자로 지정하길 권장합니다.
...
# 기존 프리셋 위치 및 이름 변경시
GET /api/channel/preset?ch=1&set=preset1&name=room
```
...
클라이언트 측에서 이벤트 트리거를 입력할 수 있습니다.
입력된 트리거는 이벤트로 기록되고 검색해 볼 수 있습니다.
또한 이벤트 트리거 발생 구간의 녹화 영상을 백업 전용 스토리지에 백업할 수 있는 이벤트 백업을 지원합니다.
서버에서는 이벤트 발생 후 녹화 (`postAlarm`) 구간까지 일반 녹화가 진행된 이후 자동으로 이벤트 백업이 시작됩니다.
`preAlarm`, `postAlarm`은 각각 최대 1시간입니다.
...
- 요청 데이터
이벤트 트리거 세부 항목을 지정할 수 있습니다.
```jsx
{
  "chid": 1,                              // 채널 번호
  "timestamp": "2022-11-29T10:13:09.646"  // (Option) 지정하지 않으면 서버 수신 시각 적용
  "title": "화재 발생",                    // 제목 (검색 결과에 표시됨)
  "comment": {                            // 주석 (내용은 자유 형식)
    "location": "101동 5층",
    "tel": "119",                 
  },
  "tasks": [
    {
      "command": "presetGo",                // (Option) PTZ 프리셋 이동
      "token": "preset1"                    // 프리셋 토큰
    },
    {
      "command": "eventBackup",             // (Option) 이벤트 영상 백업 (이 항목을 지정한 경우만 영상 백업 수행)
      "preAlarm": "5s",                     // 이벤트 발생 전 녹화 구간, 숫자 표기시 초 단위로 간주 (예: 60 이면 60초를 의미함)
      "postAlarm" "1m",                     // 이벤트 발생 후 녹화 구간
      "chids": [1, 2, 3, 4]                 // (Option) 백업할 채널 번호 (자기 자신은 지정하지 않아도 기본으로 포함됨)
    }
  ]
}
```
...
요청에 대해 서버는 다음과 같이 HTTP 응답 코드와 함께 JSON 데이터로 저장될 동영상의 URL을 응답합니다.

```jsx
{
  "eventId": 1234,  // 저장된 이벤트 id
  "videoURLs": [    // 백업될 동영상 url
...
```
같은 디렉토리에 동일한 파일이 이미 존재하는 경우, 중복 방지를 위해 아래와 같이 번호가 붙여집니다.
```jsx
{
  "eventId": 1234,  // 저장된 이벤트 id
  "videoURLs": [    // 백업될 동영상 url
...
먼저 응답을 보낸 후 이벤트 백업은 `postAlarm`으로 예약된 시점에 시작되므로 
클라이언트에서는 응답받은 URL로 즉시 접근할수 는 없고 백업이 완료된 이후(`postAlarm` 기준 약 2분 경과 후)부터 동영상 파일에 접근할 수 있습니다.

응답 데이터의 `videoURL`에 포함된 파일명은 서버 측 로컬 타임 기준으로 생성되며 아래와 같은 형식입니다.
```ruby
ch1-20221129-101309.mp4
-+- ----+--- ---+--
 |      |       |
 |      |       +------------ HHmmss (시, 분, 초 각 2자리)
 |      +-------------------- YYYYMMDD (년도 4자리, 월, 일 각 2자리)
 +--------------------------- 채널 번호 (ch 이후 가변 길이)
```
